...
// Invalidate a potentially cached TLB entry for a _guest physical_
// mapping in EPT.  This takes a physical address of the EPT root
// pointer or-ed with the relevant flags, which is stored in
// e->env_vmxinfo.eptrt as part of vm setup.
//
// Context is defined as single (one VM) or global (all VMs).
//
// This is a no-op when the CPU is not in vmx root mode, for convenience.
//
// Returns 0 on success, 1 on failure.
int
invept(eptrt_t eptrt, int64_t context)
{
	// If we are in vmx root mode, shoot down TLB entries for the guest physical address.
	// This is not relevant when creating a VM that has not run yet.
	if(thiscpu->is_vmx_root) {
		struct invept_desc desc = {eptrt, 0};
		uint8_t err = 0;

		// The low bits of eptrt are set. Catch accidentally passing cr3
		EPTRT_VALID(eptrt);

		// Check whether the CPU actually supports this instruction and arguments
		uint64_t vpid_caps_msr = read_msr(IA32_VMX_EPT_VPID_CAP);
		if (!BIT(vpid_caps_msr, 20)) {
			panic("This CPU does not support the INVEPT instruction.  Something is horrendously wrong.\n");
		}

		if (!BIT(vpid_caps_msr, 26)) {
			panic("This CPU does not support the all-context INVEPT instruction variant.  Something is horrendously wrong.\n");
		}

		if (context == INVEPT_SINGLE_CONTEXT) {
			if (!BIT(vpid_caps_msr, 25)) {
				context = INVEPT_GLOBAL;
			}
		}

#ifdef __GCC_ASM_FLAG_OUTPUTS__
		__asm __volatile("invept %2, %1" :
				 "=@ccbe"(err)
				 : "r" (context), "m"(desc) : "memory");
#else
		__asm __volatile("invept %2, %1;"
				 "setbe %0;"
				 : "=r"(err)
...
// Add Page pp to a guest's environtment e's EPT at guest physical
//  address gpa with permission perm.
//
...
// change something!  Here, note that we are changing guest physical mappings,
// and you should use the invept() function, rather than tlb_invalidate.
//
...
int test_ept_map(void)
{
	...
	if ((r = ept_page_insert(dstenv, pp, UTEMP, __EPTE_READ, 0)) < 0)
	...
	if ((r = ept_page_insert(dstenv, pp, UTEMP, __EPTE_READ, 1)) < 0)
