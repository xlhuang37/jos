...
handle_rdmsr(struct Trapframe *tf, struct VmxGuestInfo *ginfo) {
	...
	if(msr == EFER_MSR) {
		...
		uint64_t val = vmcs_read64( VMCS_64BIT_GUEST_IA32_EFER );
	...
	/* Old way; may be useful for other MSRs */
...
handle_wrmsr(struct Trapframe *tf, struct VmxGuestInfo *ginfo) {
	uint64_t msr = tf->tf_regs.reg_rcx;
	/* EFER has a dedicated save area in the vmcs, rather than the general purpose MSR save area */
	if(msr == EFER_MSR) {

		uint64_t cur_val, new_val;
		// Document old way for other MSRs
		...
		cur_val = vmcs_read64( VMCS_64BIT_GUEST_IA32_EFER );

		new_val = (tf->tf_regs.reg_rdx << 32)|tf->tf_regs.reg_rax;
		if(BIT(cur_val, EFER_LMA) == 0 && BIT(new_val, EFER_LMA) == 1) {
			// Long mode active
		...
		} else if(BIT(cur_val, EFER_LME) == 0 && BIT(new_val, EFER_LME) == 1) {
			uint32_t entry_ctls = vmcs_read32( VMCS_32BIT_CONTROL_VMENTRY_CONTROLS );
			entry_ctls |= VMCS_VMENTRY_LOAD_EFER;
			vmcs_write32( VMCS_32BIT_CONTROL_VMENTRY_CONTROLS,
				      entry_ctls );
		...
		vmcs_write64(VMCS_64BIT_GUEST_IA32_EFER, new_val);
		tf->tf_rip += vmcs_read32(VMCS_32BIT_VMEXIT_INSTRUCTION_LENGTH);
		return true;
	}
...
bool
handle_eptviolation(struct Env *e) {
	...
	if(gpa < 0xA0000 || (gpa >= 0x100000 && gpa < ginfo->phys_sz))
	{
		...
		r = ept_page_insert(e, p, (void *)ROUNDDOWN(gpa, PGSIZE), __EPTE_FULL, 0);
		assert(r >= 0);
		...
		r = ept_page_insert(e, pa2page(PADDR(((void *) (KERNBASE + CGA_BUF)))),
...
bool
handle_vmcall(struct Trapframe *tf, struct Env *e)
{
	struct VmxGuestInfo *gInfo = &e->env_vmxinfo;
	bool handled = false;
